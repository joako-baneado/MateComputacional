...
def generar_red(n_paradas, n_rutas, max_conexiones):
    ...
    ruta_colors = {}  # dictionary to store route colors
    cmap = cm.get_cmap('viridis')  # choose a color map
    for i, ruta in enumerate(rutas):
        G.add_edge(*ruta)
        ruta_colors[ruta] = cmap(i / len(rutas))  # assign a color to each route

    # Dibujar grafo con NetworkX
    pos = nx.spring_layout(G)
    nx.draw_networkx_nodes(G, pos, node_size=500)
    for ruta in rutas:
        nx.draw_networkx_edges(G, pos, edgelist=[ruta], edge_color=ruta_colors[ruta])
...
comp_temp = []
componentesconexas(0, matriz_adyacencia, 1, comp_temp)
print(comp_temp)

matriz_adyacencia = normalize(matriz_adyacencia)
